...
`CFBundleVersion` entries in the target's `info.properties` in
...
that pair has been used before.
...
with an **Apple Distribution** certificate and an App Store provisioning
profile instead.
...
| `IOS_APP_PROFILE_BASE64` | App Store provisioning profile for `me.byjp.KimbiaSync`, base64-encoded. |
...
job. The profile name is read from the profile itself.
...
The profile must be of type **App Store Connect** (distribution) and must be
regenerated whenever the certificate or the App ID's capabilities change.

1. Make sure the App ID exists with **HealthKit** (including background
   delivery) enabled, as described in [SETUP.md](SETUP.md).
...
3. Encode it: `base64 -i "Kimbia_Sync_App_Store.mobileprovision" | pbcopy`
   → `IOS_APP_PROFILE_BASE64`.

The workflow installs the profile into both locations Xcode 16 and older
tooling look in, named by its UUID, and exports its name for the Fastfile,
which writes the team, manual signing and the profile name into the target
with
...
3. Creates a temporary keychain, imports the certificate, and installs the
   profile, following GitHub's guide
...
4. Runs `bundle exec fastlane ios beta` (see `fastlane/Fastfile`), which
   regenerates the project, configures manual signing, archives
...
5. Records keyless build provenance for the `.ipa` with
   ...
   (`gh attestation verify KimbiaSync.ipa --owner jphastings`).
...
You can run the same lane on a Mac with the certificate and profile in your
login keychain and the environment variables exported:

```sh
bundle install
export APPLE_TEAM_ID=… APP_PROFILE_NAME=… \
