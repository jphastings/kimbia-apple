...
final class PDSClientTests: XCTestCase {
    ...
    private static let putRecord = "https://pds.example/xrpc/com.atproto.repo.putRecord"
    ...
    func test_putRecordWritesTheRecordAtTheGivenKey() async throws {
        stub.on(Self.putRecord, json: Self.strongRefJSON)
        let client = makeClient(session: session())

        let ref = try await client.putRecord(collection: "app.example.record", rkey: "3kabc", record: TestRecord(text: "hello"))

        XCTAssertEqual(ref, StrongRef(uri: "at://did:plc:abc123xyz/app.example.record/3kabc", cid: "bafyreic"))
        let body = jsonFields(of: try XCTUnwrap(stub.requests(to: Self.putRecord).last))
        XCTAssertEqual(body["repo"] as? String, Fixtures.did)
        XCTAssertEqual(body["collection"] as? String, "app.example.record")
        XCTAssertEqual(body["rkey"] as? String, "3kabc")
        XCTAssertEqual((body["record"] as? [String: Any])?["text"] as? String, "hello")
