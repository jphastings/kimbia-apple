...
final class OAuthClientTests: XCTestCase {
    ...
    func test_signInPushesTheRequestThenOpensTheBrowserWithIt() async throws {
        ...
        XCTAssertEqual(scheme, "me.byjp.kimbia-sync")
    ...
    func test_surfacesTheServersRefusal() async {
        stub.stubIdentity()
        stub.stubOAuthDiscovery()
        stub.on(Fixtures.parEndpoint, status: 400, json: #"{"error": "invalid_scope", "error_description": "Unknown scope include:app.kimbia.authFull"}"#)
        let error = await errorThrown { try await makeClient().signIn(account: Fixtures.handle, openBrowser: approvingBrowser()) }
        XCTAssertEqual(error as? OAuthError, .authorizationServerRejected("Unknown scope include:app.kimbia.authFull"))
    }
    ...
    func test_errorsReadLikeSentences() {
        XCTAssertEqual(OAuthError.sessionExpired.localizedDescription, "Your sign-in has expired. Open Kimbia Sync and sign in again.")
