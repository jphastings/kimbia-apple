...
/// encoded.
///
/// The item is `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`: syncing
/// happens in the background, woken by HealthKit or a background refresh
/// task, and may run while the device is locked, so the tokens must be
/// readable then. `ThisDeviceOnly` keeps the DPoP private key out of iCloud
/// Keychain backup and device-to-device migration.
public final class KeychainSessionStore: SessionStore, Sendable {
    ...
    ///   - accessGroup: A Keychain access group to share the item through.
    ///     `nil` keeps it in the app's default group, which is all a single
    ///     app with no extensions needs.
