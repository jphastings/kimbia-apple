...
/// `SessionStore` so every sync (foreground or background) stays in step. The client
...
public actor PDSClient {
    ...
    /// `com.atproto.repo.putRecord`: creates the record at `rkey`, or
    /// replaces it if one is already there. Writing the same activity twice
    /// therefore leaves exactly one record, whatever happened in between.
    public func putRecord<R: Encodable>(collection: String, rkey: String, record: R) async throws -> StrongRef {
        let input = PutRecordInput(repo: did, collection: collection, rkey: rkey, record: record)
        let response = try await post("com.atproto.repo.putRecord", body: input)
        return try decode(StrongRef.self, from: response)
    }
...
final class SessionVault: @unchecked Sendable {
    ...
    /// The login to authenticate with. Several clients (a foreground sync and
    /// a background one, say) share one Keychain item, so the store may hold
    /// a newer login than the one this client last saw;
...
struct PutRecordInput<Record: Encodable>: Encodable {
    let repo: String
    let collection: String
    let rkey: String
    let record: Record
